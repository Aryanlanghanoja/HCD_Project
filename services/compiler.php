...
// session_start(); // To access $_SESSION['user']
...
// Submit code and get result in one request
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $apiUrl);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POST, true);
...
// Evaluate result directly
$statusId = $result['status']['id'] ?? null;
if ($statusId !== 3) {
    $errorMessage = base64_decode($result['stderr'] ?? $result['compile_output'] ?? "Unknown error.");
    echo "Error ({$result['status']['description']}): $errorMessage";
    exit;
}
